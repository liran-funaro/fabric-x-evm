...
- [ ] **Step 2: Decide the wait-for-commit signal** (verify-first). Two candidates already in the codebase: (a) the gateway synchronizer/`Chain.Handle` sees the committed block containing the batch's Fabric tx; (b) the `AllTxBatchDispatcher` notification path (used by the perf harness). Use whichever the production gateway already wires. Write a small `awaitCommit(ctx, fabricTxID) error` that blocks until the batch's Fabric tx is observed committed (or ctx done). If neither is directly usable, poll `store.GetTransactionByHash`/block height for the submitted Fabric tx ID with a short interval. Document the choice at the top of `executor.go`.
