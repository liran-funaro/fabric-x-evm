...
- **Drain-all loop.** A single executor goroutine repeatedly drains the pending pool as one batch. Execution concurrency therefore equals the batch size and self-tunes to load; there is no worker-count knob. An optional `maxBatchSize` bound (`Gateway.SetMaxBatchSize`) caps how many transactions one cycle folds into a single merged Fabric transaction — 0 means unbounded (pure drain-all, which assumes arrival is paced upstream, e.g. by the query-service read on the submit path). A positive bound keeps a submission burst that outpaces the drain cycle from being swallowed into one oversized committer transaction (a Fabric transaction has a hard maximum message size); the remainder simply stays pending and is picked up by the next cycle, which runs immediately after the current batch commits.
