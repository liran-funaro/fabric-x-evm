...
type Gateway struct {
	...
	Pipelined bool `mapstructure:"pipelined" yaml:"pipelined"` // overlap the concurrent warm pass of batch N+1 with the serial authoritative pass of batch N; default false (serial). Auth reopens a fresh committed view and replays a frozen warm-write snapshot for ONE batch boundary (prefetch depth 1), so it is identical in effect to serial for reads within that one-boundary window. Holds unconditionally on conflict-free workloads but only ABOVE a batch-size floor on conflict-heavy ones: measured on ec2 (20000-tx replay) synthetic wins at every batch size (1.17-1.49x) while historic (hot-key USDC) LIVELOCKS at bs<=256 (depth-1 residual: a hot key written by N, skipped by N+1, read by N+2 falls outside the window) and is a clean win at bs>=512 (1.04-1.28x; at bs=1024 both commit 20000/20000, 0 rollbacks, ~1.25x). Default stays false; if enabled, keep bs>=512. See core.Gateway.SetPipelined and report/pipeline_report.html.
}
